...
export class TetrisInterpreter {
  ...
  assert(cond: boolean, lineNumber: number, msg: string) {
    if (!cond) this.errorMessage += `Error ${lineNumber}行目:` + msg + "\n";
  ...
  execute1Step(): string {
    ...
    if (!this.code || !this.code[this.label]) return "";
    ...
    for (let y = 19; y >= 0; y--) {
      let ok = true;
      for (let x = 0; x < 10; x++) {
        if (this.state[x][y] === "") ok = false;
      }
      if (!ok) continue;
      let code = "";
      r += 100;
      for (let x = 0; x < 10; x++) code += this.state[x][y];
      codes.push(code);
      for (let x = 0; x < 10; x++) this.state[x][y] = "";
      for (let y2 = y - 1; y2 >= 0; y2--) {
        for (let x = 0; x < 10; x++) this.state[x][y2 + 1] = this.state[x][y2];
      }
      y++;
    }
    ...
    for (let c of executeCode) {
      if (c === "I") {
        ...
        for (let x = 0; x < 10; x++) {
          for (let y = 0; y < 20; y++) this.state[x][y] = "";
        }
        return output;
      } else if (c === "+") {
